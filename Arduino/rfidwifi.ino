...
AntennaOn();
    ...
    status = MFRC522_Auth(PICC_AUTHENT1A, blockAddr, sectorKeyA[blockAddr/4], serNum);        If (status == MI_OK)
    
{
    ...
    // Reader
    blockAddr = 7;    // data block 7 
    status = MFRC522_Auth(PICC_AUTHENT1A, blockAddr, sectorNewKeyA[blockAddr/4], serNum);       if (status == MI_OK)
    {
      // Read data
                        blockAddr = blockAddr - 3 ; 
    ...
    Delay(3000);      //Different for each reader       
